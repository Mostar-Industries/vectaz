...
import { Check, FileDown, Mail, Loader2, Send } from 'lucide-react';
...
const SuccessView: React.FC<SuccessViewProps> = ({
  ...
  const generatePDF = () => {
    ...
    if (rfqData.requesterName || rfqData.companyName) {
      doc.setFontSize(12);
      doc.text('Requester Details', 14, 40);
    ...
    // Use the table's final Y position if available, otherwise default to 40
    const startY = doc.previousAutoTable?.finalY + 10 || 40;
    doc.setFontSize(12);
    doc.text('Shipment Details', 14, startY);
    ...
    // Add selected forwarders if available
    if (rfqData.forwarders && rfqData.forwarders.length > 0) {
      // Use the previous table's final Y position if available
      const forwarderY = doc.previousAutoTable?.finalY + 10 || startY + 50;
    ...
    // Add footer
    // Use the previous table's final Y position if available
    const footerY = doc.previousAutoTable?.finalY + 15 || 250;
    doc.setFontSize(10);
  ...
  const handleSendEmail = async (isTest = false) => {
    // Get the appropriate email based on whether this is a test or not
    ...
    try {
      ...
      // Close the appropriate dialog and reset the email field
      if (isTest) {
